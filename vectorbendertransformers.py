...
# we silently fail the import here since message is already taken care of in
# vectorbender.py
try:
...
class BendTransformer(Transformer):
    def __init__(self, pairsLayer, restrictToSelection, buff):
        ...
        # If there is a buffer, we add a ring outside the hull so that
        # the transformation smoothly stops
        if buff > 0:
        ...
        self.delaunay = Triangulation(
            [p.x() for p in self.pointsA], [p.y() for p in self.pointsA]
        )
    ...
    def fromCartesianToTriangular(self, p, t1, t2, t3):
        """Returns triangular coordinates (l1, l2, l3) for a given point
        in a given triangle p is a duplet for cartesian coordinates
        coordinates
        """
        x, y = p
        x1, y1 = t1.x(), t1.y()
...
class AffineTransformer(Transformer):
    def __init__(self, pairsLayer, restrictToSelection):
        ...
        self.a = (x12 * (y31 - y21) - x22 * y31 + x32 * y21 + (x22 - x32) * y11) / (
            x11 * (y31 - y21) - x21 * y31 + x31 * y21 + (x21 - x31) * y11
        )
...
class LinearTransformer(Transformer):
    def __init__(self, pairsLayer, restrictToSelection):
        ...
        # scale
        self.ds = sqrt(
            (self.b2.x() - self.b1.x()) ** 2.0 + (self.b2.y() - self.b1.y()) ** 2.0
        ) / sqrt(
            (self.a2.x() - self.a1.x()) ** 2.0 + (self.a2.y() - self.a1.y()) ** 2.0
        )
        # rotation
        self.da = atan2(self.b2.y() - self.b1.y(), self.b2.x() - self.b1.x()) - atan2(
            self.a2.y() - self.a1.y(), self.a2.x() - self.a1.x()
        )
        # translation
    ...
    def map(self, p):
        ...
        # rotation
        p = QgsPointXY(
            cos(self.da) * p.x() - sin(self.da) * p.y(),
            sin(self.da) * p.x() + cos(self.da) * p.y(),
        )
