...
class VectorBenderDialog(QtWidgets.QDialog):
    ...
    def toBendLayer(self):
        """Returns the current toBend layer depending on what is choosen in
        the comboBox_pairsLayer
        """
        layerId = self.comboBox_toBendLayer.itemData(
            self.comboBox_toBendLayer.currentIndex()
        )
        return QgsProject.instance().mapLayer(layerId)

    def pairsLayer(self):
        """Returns the current pairsLayer layer depending on what is choosen
        in the comboBox_pairsLayer
        """
        layerId = self.comboBox_pairsLayer.itemData(
            self.comboBox_pairsLayer.currentIndex()
        )
        return QgsProject.instance().mapLayer(layerId)

    def bufferValue(self):
        """Returns the current buffer value depending on the input in the
        spinbox
        """
    ...
    def refreshStates(self):
        """Updates the UI values, to be used upon opening / activating
        the window
        """
    ...
    def checkRequirements(self):
        """To be run after changes have been made to the UI. It
        enables/disables the run button and display some messages.
        """
    ...
    def updateEditState_pairsLayer(self):
        ...
        self.editModeButton_pairsLayer.setChecked(
            False if (l is None or not l.isEditable()) else True
        )
    ...
    def updateTransformationType(self):
        """Update the stacked widget to display the proper transformation type.
        Also runs checkRequirements()
        """
    ...
    def toggleEditMode(self, checked, toBendLayer_True_pairsLayer_False):
        l = (
            self.toBendLayer()
            if toBendLayer_True_pairsLayer_False
            else self.pairsLayer()
        )
        if l is None:
            return
        if checked:
            ...
            if not l.isModified():
                ...
                retval = QMessageBox.warning(
                    self,
                    "Stop editting",
                    "Do you want to save the changes to layer %s ?" % l.name(),
    ...
    def createMemoryLayer(self):
        """Creates a new memory layer to be used as pairLayer,
        and selects it in the ComboBox.
        """
        suffix = ""
        name = "Vector Bender"
    ...
    def showPreview(self):
        ...
        for i, tri in enumerate(transformer.delaunay.triangles):
            ...
            self.rubberBands[2].addPoint(transformer.pointsA[tri[2]], True, i)
